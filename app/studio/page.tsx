...
import { Upload, Scan, Box, Sparkles, Loader2, ChevronRight, ChevronLeft, Check, RotateCcw, Key, ImageIcon, Wand2, Download } from 'lucide-react';
...
const STEP_LABELS: Record<Step,string> = {
  upload:'העלאה', detect:'זיהוי', select:'בחירה', generate:'יצירה', project:'הקרנה'
};
...
const STYLE_PRESETS = [
  ...
  { label: 'נייאון', prompt: 'neon cyberpunk glowing lines electric club projection' },
...
export default function StudioPage() {
  ...
  function drawDetections(ctx: CanvasRenderingContext2D, img: HTMLImageElement, regions: Region[], sel: Region|null) {
    ...
    regions.forEach(r => {
      ...
      ctx.fillStyle = isSel ? '#7c5cfc' : color+'bb';
      ctx.fillRect(x, y-26, Math.max(tw, 60), 26);
      ctx.fillStyle = '#fff';
      ctx.font = `${isSel?'bold ':''}${isSel?13:11}px sans-serif`;
      ctx.setLineDash([]);
  ...
  function handleCanvasClick(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    if (hit) {
      ...
      // Extract region as file for generation
      const tmp = document.createElement('canvas');
      tmp.width = hit.bbox.width * W; tmp.height = hit.bbox.height * H;
      const tctx = tmp.getContext('2d')!;
      tctx.drawImage(imgRef.current, hit.bbox.x*W, hit.bbox.y*H, hit.bbox.width*W, hit.bbox.height*H, 0, 0, tmp.width, tmp.height);
      tmp.toBlob(blob => { if (blob) setRegionFile(new File([blob], 'region.jpg', {type:'image/jpeg'})); }, 'image/jpeg', 0.9);
      drawDetections(canvasRef.current!.getContext('2d')!, imgRef.current, analysis.regions, hit);
    }
  }
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (data.imageUrl) {
        const newImg = { imageUrl: data.imageUrl, prompt: stylePrompt, style: stylePrompt };
        setGeneratedImages(prev => [newImg, ...prev]);
        setActiveImage(newImg);
        setStep('project');
      } else {
        setError(data.error || 'לא נוצרה תמונה');
      }
  ...
  function startProjection(ctx: CanvasRenderingContext2D, img: HTMLImageElement) {
    ...
    function frame() {
      ...
      ctx.fillRect(0, 0, W, H);
      const d = depth3D;
      // Multi-layer parallax for 3D depth
      for (let i = 3; i >= 0; i--) {
        const f = i / 3;
        const px = Math.sin(t * 0.8 + i * 0.6) * d * 18 * f;
        const py = Math.cos(t * 0.5 + i * 0.4) * d * 10 * f;
        const scale = 1 + f * d * 0.15;
        const alpha = i === 0 ? 1 : 0.12 * (1 - f) * d * 2;
        ctx.save();
        ctx.globalAlpha = alpha;
        if (i > 0) ctx.filter = `blur(${f * d * 2}px)`;
        const dw = W * 0.8 * scale, dh = H * 0.8 * scale;
        const dx = (W - dw) / 2 + px, dy = (H - dh) / 2 + py;
        if (isOval) {
          ctx.beginPath();
          ctx.ellipse(W/2 + px*0.5, H/2 + py*0.5, dw/2, dh/2, 0, 0, Math.PI*2);
          ctx.clip();
        }
  ...
  const downloadProjection = () => {
    ...
    a.download = `projection-${Date.now()}.png`;
    a.click();
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="sticky top-0 z-50 flex items-center gap-1 bg-black/95 backdrop-blur border-b border-white/10 px-4 py-3">
        <a href="/projects" className="p-2 text-gray-600 hover:text-white transition-colors mr-2">
          <ChevronLeft size={18} />
        </a>
        <span className="text-sm font-bold text-white mr-4">🎭 Studio</span>
        {STEPS.map((s,i) => (
          <div key={s} className="flex items-center">
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
              step===s ? 'bg-violet-600 text-white' :
              STEPS.indexOf(s)<STEPS.indexOf(step) ? 'text-violet-400' : 'text-gray-700'
            }`}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        {step==='upload' && (
          <div className="flex flex-col items-center justify-center min-h-[65vh] gap-8">
            <div className="text-center">
              ...
              <p className="text-gray-400 text-lg">העלאתמונה → זיהוי אזורים → Google Imagen → הקרנה תלת מימד</p>
            </div>
            <label className="cursor-pointer group">
              <div className="w-80 h-56 border-2 border-dashed border-white/15 rounded-3xl flex flex-col items-center justify-center gap-4 group-hover:border-violet-500 group-hover:bg-violet-500/5 transition-all">
                ...
                <div className="text-center">
                  <p className="text-violet-400 font-bold">לחץ לבחירת תמונה</p>
        ...
        {step==='detect' && (
          <div className="flex flex-col items-center gap-8">
            <div className="text-center">
              <h2 className="text-3xl font-bold mb-2">🔍 זיהוי אזורים</h2>
              <p className="text-gray-400">Claude Vision מנתח את התמונה ומזהה כל אזור שאפשר להקרין</p>
            </div>
            {imageUrl && <img src={imageUrl} className="max-h-72 rounded-2xl border border-white/10" alt="" />}
            <button onClick={detect} disabled={detecting}
              className="flex items-center gap-3 px-10 py-4 bg-violet-600 hover:bg-violet-700 disabled:opacity-60 rounded-2xl font-bold text-lg transition-colors">
        ...
        {step==='select' && analysis && (
          <div className="space-y-6">
            <div className="text-center">
              <h2 className="text-3xl font-bold mb-1">▢ בחר אזור להקרנה</h2>
              <p className="text-gray-400">לחץ על אזור בתמונה</p>
            </div>
            <div className="grid grid-cols-5 gap-6">
              <div className="col-span-3">
                <canvas ref={canvasRef} className="w-full rounded-2xl border border-white/10 cursor-crosshair" onClick={handleCanvasClick} />
              </div>
              <div className="col-span-2 space-y-3">
                ...
                {analysis.regions.map(r => (
                  <button key={r.id} onClick={() => {
                    ...
                    if(canvasRef.current && imgRef.current)
                      drawDetections(canvasRef.current.getContext('2d')!, imgRef.current, analysis.regions, r);
                  ...
                  className={`w-full p-3 rounded-xl border text-right transition-all ${
                    selectedRegion?.id===r.id ? 'border-violet-500 bg-violet-500/20' : 'border-white/10 bg-white/5 hover:border-violet-500/40'
                  }`}>
                    <div className="flex items-center justify-between">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                        r.projectionPotential==='high'?'bg-violet-500/30 text-violet-300':
                        r.projectionPotential==='medium'?'bg-green-500/30 text-green-300':'bg-gray-700 text-gray-400'
                ...
                {selectedRegion && (
                  <button onClick={() => setStep('generate')}
                    className="w-full py-3 rounded-xl bg-violet-600 hover:bg-violet-700 font-bold flex items-center justify-center gap-2 transition-colors">
                    <Wand2 size={16} /> בחר — המשך
                  </button>
                )}
              </div>
            </div>
        ...
        {step==='generate' && selectedRegion && (
          <div className="max-w-2xl mx-auto space-y-8">
            <div className="text-center">
              <h2 className="text-3xl font-bold mb-2">✨ יצירת סגנון</h2>
            ...
            <div className="p-5 rounded-2xl bg-white/5 border border-white/10 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-sm">Google AI Studio API Key</span>
                <a href="https://aistudio.google.com/apikey" target="_blank"
                  className="text-xs text-violet-400 hover:underline mr-auto">קבל מפתח חינמי ↗</a>
              </div>
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="AIza..."
                  className="flex-1 bg-black border border-white/20 rounded-xl px-4 py-2.5 text-sm font-mono text-white focus:border-violet-500 outline-none"
                />
                <button onClick={() => setApiKeyVisible(!apiKeyVisible)}
                  className="px-4 py-2 rounded-xl border border-white/10 text-gray-500 hover:text-white text-xs transition-colors">
                  {apiKeyVisible ? 'הסתר' : 'הצג'}
                </button>
              </div>
              <p className="text-xs text-gray-600">המפתח נשמר אצלך בדפדפן בלבד. אנחנו לא שומרים אותו.</p>
            </div>

            {/* Style preset pills */}
            <div className="space-y-3">
              <p className="text-xs text-gray-500 uppercase tracking-wider">סגנונות מוכנים</p>
              <div className="flex flex-wrap gap-2">
                {STYLE_PRESETS.map(s => (
                  <button key={s.label} onClick={() => setStylePrompt(s.prompt)}
                    className={`px-4 py-2 rounded-full text-sm border transition-all ${
                      stylePrompt===s.prompt ? 'border-violet-500 bg-violet-500/20 text-white' : 'border-white/10 text-gray-400 hover:border-violet-500/50'
                    }`}>{s.label}</button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <p className="text-xs text-gray-500 uppercase tracking-wider">או כתוב בעצמך</p>
              <textarea
                value={stylePrompt}
                onChange={e => setStylePrompt(e.target.value)}
                placeholder="פנים עם סדקים מאובקים ועיניים קוסמיות... זהב נזלי... אש בוער..."
                rows={3}
                className="w-full bg-black border border-white/10 rounded-2xl px-4 py-3 text-white placeholder-gray-700 focus:border-violet-500 outline-none resize-none text-sm"
              />
            </div>
            ...
            {regionFile && (
              <div className="flex items-center gap-4 p-4 rounded-xl bg-white/5 border border-white/10">
                ...
                <div>
                  <p className="font-semibold">{selectedRegion.label}</p>
                  <p className="text-xs text-gray-500 mt-1">האזור שנבחר יישלח ל-Google Imagen לשינוי</p>
                </div>
              </div>
            )}

            <button onClick={generate} disabled={generating || !googleApiKey || !stylePrompt}
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 disabled:opacity-40 font-bold text-lg flex items-center justify-center gap-3 transition-all">
              {generating
                ? <><Loader2 size={22} className="animate-spin" /> Google Imagen יוצר...</>
                : <><ImageIcon size={22} /> צור עם Google Imagen</>}
            </button>

            {/* Previous generations */}
            {generatedImages.length > 0 && (
              <div className="space-y-3">
                <p className="text-xs text-gray-500 uppercase tracking-wider">תמונות קודמות</p>
                <div className="grid grid-cols-3 gap-3">
                  {generatedImages.map((img,i) => (
                    <div key={i} className="relative cursor-pointer rounded-xl overflow-hidden border border-white/10 hover:border-violet-500 transition-all"
                      onClick={() => { setActiveImage(img); setStep('project'); }}>
                      <img src={img.imageUrl} className="w-full aspect-square object-cover" alt="" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-2">
                        <span className="text-[10px] text-gray-300 truncate">{img.style}</span>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {step==='project' && activeImage && (
          <div className="space-y-6">
            <div className="text-center">
              <h2 className="text-3xl font-bold mb-1">📡 הקרנה תלת מימד</h2>
              <p className="text-gray-400">אזור: <span className="text-violet-400 font-semibold">{selectedRegion?.label}</span></p>
            </div>
            <div className="relative bg-black rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
              ...
              <div className="absolute top-4 left-4 text-xs text-gray-600 font-mono">הקרנה תלת מימד — {activeImage.style}</div>
            </div>
            <div className="max-w-lg mx-auto space-y-4">
              <div>
                <div className="flex justify-between text-xs text-gray-500 mb-2">
                  <span>עומק תלת מימד</span><span>{Math.round(depth3D*100)}%</span>
                </div>
              ...
              <div className="flex gap-3">
                <button onClick={downloadProjection}
                  className="flex-1 py-3 rounded-xl bg-green-600 hover:bg-green-700 font-bold flex items-center justify-center gap-2 transition-colors">
                  <Download size={16} /> הורד PNG
                </button>
                <button onClick={() => setStep('generate')}
                  className="flex-1 py-3 rounded-xl border border-white/10 text-gray-400 hover:text-white font-semibold flex items-center justify-center gap-2 transition-colors">
                  <Sparkles size={16} /> סגנון אחר
                </button>
                <button onClick={() => { setStep('upload'); setImageUrl(null); setImageFile(null); }}
                  className="py-3 px-4 rounded-xl border border-white/10 text-gray-600 hover:text-white transition-colors">
                  <RotateCcw size={16} />
                </button>
              </div>
            </div>
